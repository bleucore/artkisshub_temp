using ArtAPI.info;
using ArtAPI.utils;
using DocuSign.eSign.Client;
using DocuSign.eSign.Client.Auth;
using Newtonsoft.Json.Linq;
using System;
using System.Collections.Generic;
using System.IO;

using System.Threading;

using System.Windows.Forms;
using Tulpep.NotificationWindow;

namespace ArtAPI.Adapter
{
    // 웹훅은 알아가야 한다....
    // 일단 FTP polling 로 이벤트처리

    public  class   DocuConfig
    {
        public  string  IntegratorKey { get; set; }
        public  string  Host { get; set; }
        public  ApiClient ApiClient { get; set; }
        //public Configuration Configuration { get; set; }
        public  string  AccountId { get; set; }
        public  string  RecipientEmail { get; set; }
        public  string  RecipientName { get; set; }
        public  string  TemplateRoleName { get; set; }
        public  string  TemplateId { get; set; }
        public  string  EnvelopeId { get; set; }
        public  string  ReturnUrl { get; set; }
        public  string  UserId { get; set; }
        public  string  OAuthBasePath { get; set; }
        public  string  PrivateKey { get; set; }
        public  int     ExpiresInHours { get; set; }

        public  List<string>    EnvelopeIdsList { get; set; }

        public  string  IntegratorKeyNoConsent { get; set; }

        public  string  PrivateKeyNoConsent { get; set; }

        public  string  BrandLogo { get; set; }

        public  string  BrandId { get; set; }

        public  DocuConfig(string integratorKey = null, string host = null, string recipientEmail = null, string recipientName = null, string templateRoleName = null, string templateId = null, string returnUrl = null)
        {
            string userIdFromEnv = Environment.GetEnvironmentVariable("userid");


            string integratorKeyFromEnv = Environment.GetEnvironmentVariable("integratorkey");
            string templateIdFromEnv = Environment.GetEnvironmentVariable("templateid");
            string integratorKeyNoConsentFromEnv = Environment.GetEnvironmentVariable("integratorkeynoconsent");
            string brandIdfromEnv = Environment.GetEnvironmentVariable("brandid");

            this.Host = (host != null) ? host : "https://demo.docusign.net/restapi";
            this.IntegratorKey = (integratorKey != null) ? integratorKey : integratorKeyFromEnv;

            this.RecipientEmail = (recipientEmail != null) ? recipientEmail : "docusignsdktest@mailinator.com";
            this.RecipientName = (recipientName != null) ? recipientName : "Pat Developer";

            this.TemplateRoleName = (templateRoleName != null) ? templateRoleName : "bob";
            this.TemplateId = (templateId != null) ? templateId : templateIdFromEnv;

            this.ReturnUrl = (returnUrl != null) ? returnUrl : "https://www.docusign.com/api";

            this.UserId = userIdFromEnv;
            this.OAuthBasePath = "account-d.docusign.com";
            this.PrivateKey = Environment.GetEnvironmentVariable("privatekey");
            this.ExpiresInHours = 1;

            this.EnvelopeIdsList = new List<string>();

            this.IntegratorKeyNoConsent = integratorKeyNoConsentFromEnv;
            this.PrivateKeyNoConsent = Environment.GetEnvironmentVariable("privatekeynoconsent");

            this.BrandLogo = "";
            this.BrandId = brandIdfromEnv;
            //this.Configuration = new Configuration();
        }
    }

    public sealed  class   DocuSignApt
    {
        private const   int     POLLING_TIME    = 5000;         
            
        public  string  IntegratorKey   = "e0335852-c367-4f1c-857f-396f59218582";
        public  string  UserID          = "ad4f9bda-bead-4b4b-9fb7-9ad0032e6cbf";
        public  string  OAuthBasePath   = "account-d.docusign.com";

        public  string  PollingURI      = "";

        public  FTPUtil mFTPUtil        = new FTPUtil();
        private Form    mDispForm;

        private DocuConfig  docuConfig  = new DocuConfig();

        public  JArray  mDocuList       = new JArray();

        private static readonly DocuSignApt instance = new DocuSignApt();
        public static DocuSignApt Instance
        {
            get
            {
                return instance;
            }
        }

        private DocuSignApt()
        {
            //JObject obj = SignInfo.MakeTestData("id_01", "id_01");
            mDocuList.Add(SignInfo.MakeTestData("5efbe3314a1cf73df0a6c2c7", "sign_01", "id_11"));
            mDocuList.Add(SignInfo.MakeTestData("5efbe46f4a1cf73df0a6c2ca", "sign_01", "id_12"));
            mDocuList.Add(SignInfo.MakeTestData("artist_03", "sign_01", "id_13"));
            mDocuList.Add(SignInfo.MakeTestData("5efbe3314a1cf73df0a6c2c7", "sign_02", "id_21"));
            mDocuList.Add(SignInfo.MakeTestData("5efbe46f4a1cf73df0a6c2ca", "sign_02", "id_22"));
            mDocuList.Add(SignInfo.MakeTestData("artist_03", "sign_02", "id_23"));
            mDocuList.Add(SignInfo.MakeTestData("artist_04", "sign_02", "id_24"));
            mDocuList.Add(SignInfo.MakeTestData("5efbe3314a1cf73df0a6c2c7", "sign_03", "id_31"));
            mDocuList.Add(SignInfo.MakeTestData("5efbe46f4a1cf73df0a6c2ca", "sign_03", "id_32"));
            mDocuList.Add(SignInfo.MakeTestData("artist_03", "sign_03", "id_33"));
            mDocuList.Add(SignInfo.MakeTestData("artist_04", "sign_03", "id_34"));
            mDocuList.Add(SignInfo.MakeTestData("artist_05", "sign_03", "id_35"));
            mDocuList.Add(SignInfo.MakeTestData("5efbe3314a1cf73df0a6c2c7", "sign_04", "id_41"));
            mDocuList.Add(SignInfo.MakeTestData("5efbe3314a1cf73df0a6c2c7", "sign_05", "id_51"));
            mDocuList.Add(SignInfo.MakeTestData("5efbe46f4a1cf73df0a6c2ca", "sign_05", "id_52"));
            mDocuList.Add(SignInfo.MakeTestData("artist_03", "sign_05", "id_53"));

            mDocuList.ToString();
        }

        public  bool    SetSignInfo(SignInfo sInfo)
        {
            return  true;
        }

        public  SignInfo    GetSignInfo(string sign_id)
        {
            foreach (JObject docu in mDocuList) {
                if (sign_id == docu["sign_id"].ToString()) {
                    SignInfo    info    = new SignInfo();

                    info.sign_id    = sign_id;
                    info.artist_id  = docu["artist_id"].ToString();
                    info.type       = docu["type"].ToString();
                    info.title      = docu["title"].ToString();
                    info.s_date     = docu["s_date"].ToString();
                    info.e_date     = docu["e_date"].ToString();

                    info.amount     = docu["amount"].ToString();
                    info.desc       = docu["desc"].ToString();

                    JArray arr      = (JArray)docu["comps"];
                    foreach(JObject obj in arr)
                    {
                        CompInfo cInfo  = new CompInfo();
                        cInfo.Parse(obj);

                        info.comps.Add(cInfo);
                    }

                    return info;
                }
            }
            return  null;
        }

        public  List<JObject>   GetList(string a_id)
        {
            List<JObject>    list    = new List<JObject>();
            foreach(JObject docu in mDocuList) {
                //string  artist_id   = 
                if (a_id == docu["artist_id"].ToString()) {
                    list.Add(docu);
                }
            }

            return  list;
        }

        public  bool    Init(Form form)
        {
            mDispForm   = form;
            docuConfig.ApiClient = new ApiClient(docuConfig.Host);

            byte[] privateKeyStream = Convert.FromBase64String(File.ReadAllText(@"Config\docusign_private.pem"));
            try {
                OAuth.OAuthToken tokenInfo = docuConfig.ApiClient.RequestJWTUserToken(
                                    IntegratorKey, UserID, OAuthBasePath, privateKeyStream, 1);

                Console.WriteLine("{0}", tokenInfo.access_token);

                // the authentication api uses the apiClient (and X-DocuSign-Authentication header) that are set in Configuration object
                OAuth.UserInfo userInfo = docuConfig.ApiClient.GetUserInfo(tokenInfo.access_token);

                //Assert.IsNotNull(userInfo);
                //Assert.IsNotNull(userInfo.Accounts);

                foreach (var item in userInfo.Accounts) {
                    if (item.IsDefault == "true") {
                        docuConfig.AccountId = item.AccountId;
                        docuConfig.ApiClient.SetBasePath(item.BaseUri + "/restapi");
                        break;
                    }
                }

                Thread dsp = new Thread(new ParameterizedThreadStart(DocuSignProc));
                dsp.Start(this);

                return  true;
                //Assert.IsNotNull(testConfig.AccountId);
            }
            catch (Exception e) {
                Console.WriteLine(e.Message);
            }

            return  false;
        }

        public void DocuSignProc(object obj)
        {
            // retrieve client from parameter passed to thread
            DocuSignApt manager = (DocuSignApt)obj;
            //ImageUtil imageUtil = new ImageUtil();
            while (true)
            {
                
                // FTP polling
                string[] lists = mFTPUtil.GetFileList("/ArtKiss/DocuSign", "Sign");
                if (lists != null && lists.Length > 0) {
                    mDispForm.Invoke(new Action(delegate () {
                        PopupNotifier popup = new PopupNotifier();
                        popup.TitleText = "계약 발생";
                        popup.ContentText = "계약 상세 사항을 표시한다.";
                        popup.Popup();
                    }));

                    //mFTPUtil.GetFile()

                    Console.WriteLine(lists.ToString());
                }
                Thread.Sleep(POLLING_TIME);
            }
        }
    }
}