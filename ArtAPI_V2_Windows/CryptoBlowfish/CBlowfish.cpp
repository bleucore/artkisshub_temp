...
void    CBlowfish::Gen_Subkeys(char *Passwd) {
...
void    CBlowfish::BF_En(Word *x1, Word *x2) {
    Word    w1 = *x1, w2 = *x2;
    ...
    *x1 = w2;
    //LOGD("%d , %d", w1.word, w2.word);
    *x2 = w1;
}

void    CBlowfish::BF_De(Word *x1, Word *x2) {
    Word    w1 = *x1, w2 = *x2;
...
bool    CBlowfish::Set_Passwd(char *Passwd) {
...
char *  CBlowfish::Encrypt(char *data, int length)
{
    int i, new_len;
    DWord * Work    = NULL;

    byte* pData = padData((byte *)data, length, &new_len, false); //this loads the IV from the front of the ciphertext
    ...
    char *  retv;
    i   = base64_encode((char *)pData, new_len * 8, &retv);
    delete  pData;
...
char *  CBlowfish::Decrypt(char *encrypt, int length, int *decrypt_len)
{
    int i, newlength;
    DWord * Work;
    byte * data	= new byte[length];
    i = base64_decode((char *)encrypt, &data[0], length);
    byte* pData = padData((byte *)data, i, &newlength, true); //this loads the IV from the front of the ciphertext
	delete data;
    ...
    *decrypt_len = findPaddingEnd(pData, newlength*8);

    pData[*decrypt_len] = 0x00;
    Work = NULL;
    return  (char *)pData;
}

int     CBlowfish::findPaddingEnd(byte* data, int length) {
    int i = length;
    //find the first character from the back that isnt the same as the last character
...
byte *  CBlowfish::padData(byte* data, int length, int* paddedLength, bool decrypt, bool IvSpace)
{
    int     offset      = 0;
    int     dataoffset  = 0;
    if (decrypt) {
