...
BLOWFISH::BLOWFISH(std::string hexKey)
{
	...
	SetupKey(key, (int)hexKey.length() / 2);
}
...
std::string BLOWFISH::Encrypt_CBC(std::string data)
{
	...
	for (int i = 0; i < data.length(); i++)
		binaryData[i] = data[i];
	int newlen = 0;
	byte* result = Encrypt_CBC(binaryData, (int)data.length(), &newlen);
	std::string encoded = "";
...
std::string BLOWFISH::Decrypt_CBC(std::string data)
{
	...
	byte binaryData[2048];
	for (int i = 0; i < data.length() / 2; i++)
	{
		binaryData[i] = hex2dec(data[i * 2]) * 16 + hex2dec(data[i * 2 + 1]);
	}
	int len = 0;
	byte* cryptresult = Decrypt_CBC(binaryData, (int)data.length() / 2, &len);
	std::string result = "";
	for (int i = 0; i < len; i++)
